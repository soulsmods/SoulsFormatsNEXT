...
{
    public partial class TAE : SoulsFile<SoulsFormats.Formats.TAE.TAE>
    {
        ...
        public class Animation
        {
            ...
            /// </summary>
            public List<TAE.Event> Events;

            /// <summary>
            /// Unknown groups of events.
            /// </summary>
            public List<TAE.EventGroup> EventGroups;
            ...
            public Animation(long id, AnimMiniHeader miniHeader, string animFileName)
            {
                ...
                Events = new List<TAE.Event>();
                EventGroups = new List<TAE.EventGroup>();
            }
                ...
                out long lastEventParamOffset)
            {
                ...
                br.StepIn(offset);
                {
                    ...
                    Events = new List<TAE.Event>(eventCount);
                    br.StepIn(eventHeadersOffset);
                    {
                        for (int i = 0; i < eventCount; i++)
                        {
                            eventHeaderOffsets.Add(br.Position);
                            Events.Add(TAE.Event.Read(br, out long pOffset, format));
                    ...
                    EventGroups = new List<TAE.EventGroup>(eventGroupCount);
                    br.StepIn(eventGroupsOffset);
                    {
                        for (int i = 0; i < eventGroupCount; i++)
                            EventGroups.Add(new TAE.EventGroup(br, eventHeaderOffsets, format));
                    }
                    br.StepOut();
            ...
            internal Dictionary<float, long> WriteTimes(BinaryWriterEx bw, int animIndex, TAEFormat format)
            {
                ...
                foreach (TAE.Event evt in Events)
                {
