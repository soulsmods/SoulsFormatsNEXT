...
namespace SoulsAssetPipeline.FLVERImporting
{
    public class FLVER2Importer : IDisposable
    {
        ...
        {
            context.PropertyConfigurations.Add("PP_SBBC_MAX_BONES", new Assimp.Configs.IntegerPropertyConfig("PP_SBBC_MAX_BONES", 30));
        ...
        public ImportedFLVER2Model ImportFromAssimpScene(Scene scene, FLVER2ImportSettings settings)
        {
            ...
            flver.Nodes = metaskeleton.Nodes;
                ...
                foreach (var kvp in texChanDefs)
                {
                    if (kvp.Key.Index == 0)
                    {
                        if (kvp.Key.Semantic == TextureChannelSemantic.Diffuse)
                            ...
                            AddTextureSlot(settings.Game == SoulsGames.DS1R ? material.TextureReflection : material.TextureSpecular, kvp.Value);
            ...
            foreach (var mesh in scene.Meshes)
            {
                ...
                flverMesh.NodeIndex = flver.Nodes.Count;
                int flverLastRootBoneIndex = flver.Nodes.FindLastIndex(b => b.ParentIndex == -1);
                // Register this new bone as a sibling.
                if (flverLastRootBoneIndex >= 0)
                    flver.Nodes[flverLastRootBoneIndex].NextSiblingIndex = (short)flverMesh.NodeIndex;
                flver.Nodes.Add(new FLVER.Node()
                {
                    Name = mesh.Name,
                    ...
                    BoundingBoxMax = NVector3.One * 1f,
                ...
                for (int i = 0; i < mesh.VertexCount; i++)
                {
                    ...
                    var vertTransMat = flverSceneMatrix;

                    newVert.Normal = NVector3.Normalize(NVector3.TransformNormal(mesh.Normals[i].ToNumerics(), vertTransMat));
                    ...
                    if (mesh.HasTangentBasis)
                    {
                        ...
                        var bitanXYZTransformed = NVector3.Normalize(NVector3.TransformNormal(bitanXYZ.ToNumerics(), vertTransMat));

                        //var tanW = Vector3D.Dot(Vector3D.Cross(bitanXYZ, mesh.Normals[i]), tan) >= 0 ? 1 : -1;
                        var finalBitan = new System.Numerics.Vector4(bitanXYZTransformed, bitanW);
                ...
                {
                    var bonesInMesh = mesh.Bones.ToList();
                ...
                foreach (var bone in mesh.Bones)
                {
                    ...
                    {
                        flver.Nodes[boneIndex].Flags = FLVER.Node.NodeFlags.Bone;
                    }
                    else
                    {
                    ...
                    foreach (var weight in bone.VertexWeights)
                    {
                        ...
                        //testing 
                        //flverMesh.Vertices[weight.VertexID].BoneWeights[0] = 1;
                        //flverMesh.Vertices[weight.VertexID].BoneWeights[1] = 0;
                        //flverMesh.Vertices[weight.VertexID].BoneWeights[2] = 0;
                        //flverMesh.Vertices[weight.VertexID].BoneWeights[3] = 0;
                    }
                }
                ...
                for (int i = 0; i < flverMesh.Vertices.Count; i++)
                {
                    ...
                    for (int j = 0; j < 4; j++)
                    {
                        ...
                        if (flverMesh.Vertices[i].BoneIndices[j] < 0 ||
                            (usesIndirectBones && flverMesh.Vertices[i].BoneIndices[j] >= flverMesh.BoneIndices.Count)
                            || (!usesIndirectBones && flverMesh.Vertices[i].BoneIndices[j] >= flver.Nodes.Count))
                        {
                            flverMesh.Vertices[i].BoneIndices[j] = 0;
                            flverMesh.Vertices[i].BoneWeights[j] = 0;
                        }
                    }
            ...
            //{
            //    var facesetsList = m.FaceSets.ToList();

            //    m.FaceSets.Clear();

            //    foreach (var fs in facesetsList)
            //    {
            //        List<int> boneIndicesInThisFS = new List<int>();

            //        var newFS = new FLVER2.FaceSet(fs.Flags, false, fs.CullBackfaces, fs.Unk06, new List<int>());

            //        void commitFS()
            //        {
            //            if (newFS.Indices.Count > 0)
            //                m.FaceSets.Add(newFS);

            //            newFS = new FLVER2.FaceSet(fs.Flags, false, fs.CullBackfaces, fs.Unk06, new List<int>());
            //            boneIndicesInThisFS.Clear();
            //        }

            //        List<int> getBonesInVertex(int v)
            //        {
            //            var r = new List<int>();
            //            for (int i = 0; i < 4; i++)
            //            {
            //                if (m.Vertices[v].BoneWeights[i] != 0 && !r.Contains(m.Vertices[v].BoneIndices[i]))
            //                    r.Add(m.Vertices[v].BoneIndices[i]);
            //            }
            //            return r;
            //        }

            //        List<int> getBonesInTriangle(int v1, int v2, int v3)
            //        {
            //            var r = new List<int>();
            //            foreach (var w in getBonesInVertex(v1))
            //            {
            //                if (!r.Contains(w))
            //                    r.Add(w);
            //            }
            //            foreach (var w in getBonesInVertex(v2))
            //            {
            //                if (!r.Contains(w))
            //                    r.Add(w);
            //            }
            //            foreach (var w in getBonesInVertex(v3))
            //            {
            //                if (!r.Contains(w))
            //                    r.Add(w);
            //            }
            ...
            //        bool canAddTriangleWithoutGoingOver(int v1, int v2, int v3)
            //        {
            //            var r = new List<int>();
            //            foreach (var w in boneIndicesInThisFS)
            //            {
            //                if (!r.Contains(w))
            //                    r.Add(w);
            //            }
            //            foreach (var w in getBonesInTriangle(v1, v2, v3))
            //            {
            //                if (!r.Contains(w))
            //                    r.Add(w);
            //            }
            ...
            //        for (int i = 0; i < fs.Indices.Count; i += 3)
            //        {
            //            if (!canAddTriangleWithoutGoingOver(fs.Indices[i], fs.Indices[i + 1], fs.Indices[i + 2]))
            //                commitFS();

            //            newFS.Indices.Add(fs.Indices[i]);
            //            newFS.Indices.Add(fs.Indices[i + 1]);
            //            newFS.Indices.Add(fs.Indices[i + 2]);

            //            var bones = getBonesInTriangle(fs.Indices[i], fs.Indices[i + 1], fs.Indices[i + 2]);
            //            foreach (var w in bones)
            //            {
            //                if (!boneIndicesInThisFS.Contains(w))
            //                    boneIndicesInThisFS.Add(w);
            //            }
            //        }
